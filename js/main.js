...
function burstPetals(cx, cy) {
  var colors = ['#f5c4aa','#e8b84b','#f5d98a','#e8956d','#fce4d6','#f9d5e5','#f5e6d3'];
  var count  = 40;
  for (var i = 0; i < count; i++) {
    (function(idx) {
      setTimeout(function() {
        ...
        var angle = (idx / count * 360) + (Math.random() - 0.5) * 25;
        ...
        var rot   = ((Math.random() - 0.5) * 600) + 'deg';
        petal.style.cssText =
        ...
        setTimeout(function() { if (petal.parentNode) petal.remove(); }, 2000);
      }, idx * 18);
    })(i);
  }
}
...
function openEnvelope(e) {
  ...
  var hint    = overlay.querySelector('.env-hint');
  var rect    = wrap.getBoundingClientRect();
  var cx = rect.left + rect.width / 2;
  var cy = rect.top  + rect.height * 0.46;

  hint.style.opacity = '0';
  burstPetals(cx, cy);
  wrap.classList.add('opening');

  setTimeout(function() { wrap.classList.add('lift-up'); }, 500);
...
function initMusic() {
  ...
  // start on first touch/click — only after envelope is opened
...
function initScrollReveal() {
  ...
  var targets = document.querySelectorAll(
    '#video-intro > *, #story > *, #countdown > *, #schedule > *, #venue > *, ' +
    '.story-item, .timeline li, .count-box, .dress-inline, .venue-wrap'
  );
...
function shareInvite() {
  ...
  if (navigator.share) {
    navigator.share({ title: 'Praew & Ice — Wedding 10/10/2569', url: url });
    return;
  }
